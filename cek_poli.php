...
if(isset($_GET['poli']) && !empty($_GET['poli'])) { 
	...
	$result = $koneksi -> query("select * from antrian_online_format_inc, dokter where tgl >= now() and antrian_online_format_inc.dokter = dokter.nama and dokter.poli = '".$_SESSION['poli']."' and cib = '".$_SESSION['id']."' LIMIT 1");
	if($row = $result -> fetch_array()){ 
		...
		$_SESSION['jam'] = strtotime($row['jam']);		
